...
.PHONY: prepare-e09-traces audit-e09-readiness render-e09-local serve-e09-local bench-e09-calibration-unbounded bench-e09-calibration-fixed bench-e09-calibration-deadline bench-e09-matrix bench-e09-matrix-resume compare-e09
...
prepare-e09-traces:
	$(QSL) prepare-e09-traces --config $(E09_CONFIG)

audit-e09-readiness:
	$(QSL) audit-e09-readiness --config $(E09_CONFIG)
...
bench-e09-calibration-unbounded:
	$(QSL) run-e09 --config $(E09_CONFIG) --profile calibration_burst --policy unbounded --repetition 1 --tokenizer-path "$(MODEL_PATH)"

bench-e09-calibration-fixed:
	$(QSL) run-e09 --config $(E09_CONFIG) --profile calibration_burst --policy fixed_concurrency --repetition 1 --tokenizer-path "$(MODEL_PATH)"

bench-e09-calibration-deadline:
	$(QSL) run-e09 --config $(E09_CONFIG) --profile calibration_burst --policy deadline_aware --repetition 1 --tokenizer-path "$(MODEL_PATH)"

bench-e09-matrix:
	$(QSL) run-e09-matrix --config $(E09_CONFIG) --tokenizer-path "$(MODEL_PATH)"

bench-e09-matrix-resume:
	$(QSL) run-e09-matrix --config $(E09_CONFIG) --tokenizer-path "$(MODEL_PATH)" --skip-completed

compare-e09:
	$(QSL) compare-e09 --config $(E09_CONFIG)

charts:
	$(PYTHON) scripts/generate_readme_charts.py

check-charts:
	$(PYTHON) scripts/generate_readme_charts.py --check
